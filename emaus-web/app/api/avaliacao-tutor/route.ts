// Proxy autenticado pra avaliação do Tutor na prova. Mesma trava: user_id sempre vem da sessão,
...
export async function POST(req: NextRequest) {
  ...
  const { avaliacaoId, resumoTexto } = await req.json().catch(() => ({}));
  if (!avaliacaoId || !resumoTexto) {
  ...
  const res = await fetch(`${API_URL}/pipeline/avaliacoes/${avaliacaoId}/avaliar`, {
    method: "POST",
