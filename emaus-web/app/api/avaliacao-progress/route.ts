// Proxy autenticado pra marcar progresso de prova. O user_id NUNCA vem do corpo da
...
export async function PUT(req: NextRequest) {
  ...
  const { avaliacaoId, status } = await req.json().catch(() => ({}));
  if (!avaliacaoId || !status) {
  ...
  const res = await fetch(`${API_URL}/avaliacao-progress/${avaliacaoId}`, {
    method: "PUT",
