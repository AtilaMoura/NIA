...
## Fase 0b — Nível Tópico (Módulo → Aula → Tópicos)
**Status: [OK]** — 2026-08-26. Construído a pedido do usuário no curso de obreiro (ver `curso de obreiro/anotação para IA.md`): a estrutura antiga (`Course → Module → Lesson`, `Lesson.content` = conteúdo inteiro) não tinha espaço pra "1 aula = vários tópicos, cada um com geração/revisão própria" — era só um campo `topicos` (lista de texto) dentro do JSON de estrutura, sem tabela própria.

**Construído:**
- `Topico` (`backend/app/models/models.py`) — tabela nova, FK pra `lessons` (que passa a representar a **aula**): `topico_index`, `titulo`, `referencia_biblica`, mais os mesmos campos de conteúdo/geração/revisão que `Lesson` já tinha (`content`, `generated_by`, `reviewed_by`, `review_feedback`, `is_approved`, `estimated_read_time_minutes`). `Lesson.content` **não foi removido nem deixou de funcionar** — continua valendo pra aulas que não precisam desse nível extra (ex.: cursos de tech já existentes).
- `backend/app/routers/topicos.py` — CRUD completo + `GET /topicos/{id}/render?theme=...`, espelhando `lessons.py` 1:1 (mesmo `render_topico()` reaproveitado, sem mudar nada no renderizador). Registrado em `main.py`.
- Tabela criada manualmente no Postgres (mesmo padrão sem Alembic do resto do projeto — `CREATE TABLE` direto).

**Migração real feita** (curso de obreiro, aula 1 do módulo 1, `Lesson.id=57`): 5 linhas inseridas em `Topico` — Tópico 1 "Sacerdócio de Todos os Crentes" (1Pe 2:9, conteúdo **copiado**, não movido, de `Lesson.content` — o `Lesson.content` original continua intacto), Tópico 2 "Dons Espirituais" (Rm 12:6-8, vazio, próximo a gerar), Tópico 3 "Vocação e Ofícios" (Ef 4:11-12), Tópico 4 "Função no Corpo de Cristo" (1Co 12:12-27), Tópico 5 "Integração dos Três" (síntese, sem referência nova). `Lesson.title` da 57 devolvido pro título real da aula, "O Chamado do Obreiro" (tinha sido renomeado por engano pro título do Tópico 1 antes dessa fase existir).

**Teste realizado**: `GET /topicos/?lesson_id=57` lista as 5 linhas corretamente; `GET /topicos/1/render?theme=trigo-maduro` → 200, HTML idêntico ao que `/lessons/57/render` já produzia; `GET /topicos/2/render` (sem conteúdo ainda) → 409 com mensagem clara, não erro solto.

## Fase 2c — Modo Pesquisa/Integral (novo, decidido em 2026-08-26)

**Status: [ ]** — em uso manual desde o Tópico 1 do curso de obreiro (curso_id 8), ainda não automatizado.

Motivação: o Modo Comum e o Modo Pro (Fase 2/2b) sempre geram dentro de um teto implícito de tamanho — bom pra cursos técnicos, mas insuficiente quando o usuário pede conteúdo "sem limite de slides" e fiel a uma pesquisa rica (caso real: curso de obreiro, domínio teológico, onde cortar profundidade histórica/bíblica é inaceitável). Registrado em `curso de obreiro/anotação para IA.md` (2026-08-26).

**Limite técnico que motiva o desenho em 4 estágios** (confirmado no código, não suposição): Groq (`openai/gpt-oss-120b`, tier grátis) tem teto de 8000 tokens/minuto **por requisição** (prompt + `max_tokens` somados) — fisicamente incompatível com gerar um tópico rico numa chamada só. Gemini (`gemini-2.5-flash`) aguenta mais tokens/min, mas deu timeout 504 duas vezes tentando gerar um tópico inteiro de uma vez. Só dá pra não limitar o tamanho quebrando a geração em várias chamadas pequenas.

**Os 4 estágios:**
1. **Pesquisa** — Firecrawl + `biblia_service` (ou fonte equivalente por domínio), sem limite de tamanho. Diferente do que era feito antes: **o material de pesquisa bruto precisa ser guardado** (arquivo ou campo novo), não só o resumo final — foi a perda desse material bruto que causou conteúdo mais raso que o devido no Tópico 1.
2. **Rascunho completo** — várias chamadas pequenas (Groq e/ou Gemini, misturados conforme o orçamento de token de cada seção), sem cortar. Aprovação humana (autor do curso + revisor de domínio) acontece aqui, no texto corrido, antes de virar slide.
3. **Conversão pra slide** — quase mecânica já que o texto do estágio 2 está aprovado: divide por seção, versículo/citação vira bloco `quote` próprio, sem inventar fato novo. `QuizAgent` roda em cima normalmente.
4. **Auditoria de completude/alucinação** — compara o slide final **contra o material do estágio 1** (não só contra si mesmo, que é tudo que o `ReviewerAgent` faz hoje). Decidido fazer **manual (eu revisando) por enquanto**, com plano de virar um `AuditorAgent` de IA depois que o padrão estiver validado em mais tópicos; a revisão humana dupla (autor + especialista de domínio, ver Fase T3 do plano multi-tenant) continua acontecendo em paralelo, não substitui essa auditoria técnica.

**Teste [OK quando]:** gerar um tópico pelo fluxo completo (Tópico 2 do curso de obreiro, "Dons Espirituais", é o primeiro teste real) e confirmar que nenhum fato pesquisado no estágio 1 ficou de fora do slide final.

**Atualização 2026-08-26 — Tópico 2 gerado, bugs reais de código corrigidos no caminho:**
- `GeminiService` migrado de `google-generativeai==0.3.2` (descontinuado, dava `504 Deadline Exceeded` fixo em ~60s com `gemini-2.5-flash`, bug documentado do SDK antigo) pra `google-genai==2.20.0` — interface pública idêntica, nada mais mudou. **Achado de operação**: tier grátis do Gemini tem teto de **20 requisições/dia por projeto/modelo** (bem mais restritivo que o problema de tokens/minuto do Groq) — bate rápido se Gemini virar padrão pra várias etapas.
- `QuizAgent` não cortava o conteúdo antes de mandar pro modelo (diferente do `ReviewerAgent`, que já cortava `svg_raw`) — tópico rico (44 blocos) mandou 14628 tokens numa chamada só contra o teto de 8000/min do Groq. Corrigido: `_resumir_blocos_para_quiz()` novo (corta cada campo de texto em 100 chars só no resumo mandado pra IA, o conteúdo salvo continua completo) + `BaseAgent.run_json_com_retry()` ganhou `max_tokens` opcional repassado pro service (`QuizAgent` pede só 2800, não o default de 6000).
- `montar_topico()` não repassava `imagem_capa` do conteúdo pro tópico final — só não tinha aparecido antes porque o Tópico 1 foi montado à mão, nunca passou pela função de verdade. Corrigido.
- Achado de comportamento (não é bug de código): pedindo 3 checkpoints + 5 perguntas de avaliação numa chamada só, o Groq esqueceu consistentemente 1 checkpoint (JSON bem-formado, só faltando a chave) — não resolve com mais `max_tokens`. Sem correção de código ainda; contornado manualmente uma vez.

## Ajuste na Fase 2/3 — `EstruturaAgent` ganha `PerfilDominio` (decidido em 2026-08-26)

Hoje só `ContentAgent`/`ReviewerAgent`/`TutorAgent` recebem `PerfilDominio` (`backend/app/agents/perfis.py`) — `EstruturaAgent` (monta a grade módulo→aula→tópico) continua genérico, por isso a grade do curso de obreiro foi montada inteira à mão. Ajuste: enriquecer `PerfilDominio` com `especialidade_grade` (instrução de domínio pra montagem da grade) e `fontes_preferidas` (quais referências/comentaristas priorizar no Estágio 1 acima), e passar a injetar esse perfil também no `EstruturaAgent`. Isso é a "equipe de agentes reaproveitável por curso" pedida pelo usuário — mesmo código, perfil carregado por curso, sem criar N implementações de agente.

## Ordem recomendada

0 → 1 → 2 seguem em sequência (cada uma depende da anterior). 3 e 4 podem ser feitas em paralelo depois da 2. 5 e 6 podem ser feitas em paralelo depois da 0. 7 é sempre por último. 2c é paralela à 2/2b (mesma dependência), usada quando o curso pedir profundidade sem teto de tamanho.

---

## Backlog (fora do escopo atual — só registrado, não implementar ainda)

- **Busca de imagem automática via API** (decidido em 2026-08-26, ainda fora do MVP — ver `curso de obreiro/anotação para IA.md`): caminho escolhido é a **API de busca do Pexels** primeiro (gratuita, já validada por uso manual no curso de obreiro — licença livre confirmada), não geração por IA — `ImageService`/OpenAI `gpt-image-1` já existe no código mas está bloqueado por billing. Requisito de design confirmado com o usuário: a busca deve trazer **várias opções (3-5) pra escolha humana**, nunca a IA escolhendo sozinha qual foto entra. Alternativas gratuitas de geração por IA (não-OpenAI) ficam como levantamento a fazer, sem opção travada ainda. Enquanto não automatizado, segue manual (busca + download à mão, como no Tópico 1 do curso de obreiro).
...
- **Tipo de conteúdo "prática/exercícios"** (registrado em 2026-08-13, a pedido do usuário): o schema/renderer atual (`docs/schema/schema-conteudo-topico.md`, Fase 0-1) é um slide-deck com checkpoints — bom pra conteúdo conceitual (definição → analogia → aplicação → checkpoint), mas cada pergunta aparece uma única vez, sem repetição nem fila de exercícios. Pra assuntos que pedem prática intensiva (ex.: idiomas — quanto mais exercício, melhor) isso é insuficiente. **Recomendação**: não encaixar no schema de `topico` existente (misturar os dois deixaria ambos piores — um inchado de campos só relevantes pra prática, o outro preso à lógica linear de slide). Criar um tipo de conteúdo **irmão**, ex. `pratica`, escolhido por curso ou por módulo, reaproveitando o que já vale a pena (tokens de tema, o padrão Reviewer/Tutor pra correção semântica de resposta aberta, a arquitetura de geração via JSON) mas com mecânica de **fila de exercícios com repetição** (errou → volta pra fila; sem gate linear "responde uma vez e destrava") em vez de slides lineares — mais parecido com prática tipo Duolingo do que aula expositiva. Não maduro o bastante pra entrar nas fases atuais; revisitar depois que o fluxo aluno/admin (frontend) estiver ponta a ponta.
